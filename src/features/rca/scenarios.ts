...
export type Scenario = {
  ...
  probes?: { title: string; description: string; sql: string }[];
};
...
const scenarioC: Scenario = {
  ...
  probes: [
    { title: "Expand to 28-day baseline", description: "Control for post-holiday issuer behavior.", sql: "SELECT issuer_country, date_trunc('day', timestamp), AVG(status='authorized') FROM transactions WHERE timestamp >= current_date - INTERVAL 28 DAY GROUP BY ALL;" },
    { title: "Segment by issuer BIN", description: "Search for low-volume issuer clusters.", sql: "SELECT bin_country, card_brand, decline_code, COUNT(*), AVG(status='authorized') FROM transactions GROUP BY ALL HAVING COUNT(*) > 100;" },
    { title: "Compare merchant mix", description: "Test whether portfolio composition shifted.", sql: "SELECT merchant_id, COUNT(*) volume, AVG(status='authorized') auth_rate FROM transactions GROUP BY merchant_id ORDER BY volume DESC;" },
  ],
};
